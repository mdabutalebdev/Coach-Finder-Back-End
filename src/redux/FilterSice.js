...
export const fetchGroups = createAsyncThunk("fetchGroups", async () => {
  const rsl = await axios.get(
    "http://77.37.74.82:5000/api/groups/get-all-groups"
  );
...
export const searchFeatch = createAsyncThunk("searchFeatch", async (name) => {
  const rsl = await axios.get(
    `http://77.37.74.82:5000/api/groups/get-all-groups?group_name=${name}`
  );
  let res = await rsl.data;
...
export const deleteGroup = createAsyncThunk("deleteGroup", async (id) => {
  ...
  const rsl = await axios.delete(
    `http://77.37.74.82:5000/api/groups/delete-group/${id}`,
    {
      headers: {
...
export const { toastFunc, sortFunc } = FilterSlice.actions;
